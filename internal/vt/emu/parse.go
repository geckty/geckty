...
// are delegated to t.osc52, if set (see OSC52Handler's doc comment for why
// this hook exists — upstream cy/pkg/emu parses OSC 52 correctly but has
...
func (t *State) handleOSC52(s strEscape) {
	...
	if pd == "?" {
		if t.osc52 == nil {
			t.logf("osc-52: query not supported (no OSC52Handler installed)")
			return
		}
		data, ok := t.osc52.OSC52Read(pc)
		if !ok {
			return
		}
		_, _ = fmt.Fprintf(t.w, "\033]52;%s;%s\033\\", pc, base64.StdEncoding.EncodeToString(data))
		return
	}
