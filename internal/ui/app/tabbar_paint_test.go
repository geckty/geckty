...
func TestPaintTabAndPlusButton(t *testing.T) {
	...
	buf := newBuf(200, 40)
	tb.paintTab(buf, 200, 40, thm, tab, 0, 100, 32, 3, true, false, false, true, true)

	// Plus button paints its own chip + cross.
	tb.paintPlusButton(buf, 200, thm, 150, 36, 32, 3, true)
	tb.paintPlusButton(buf, 200, thm, 150, 36, 32, 3, false)
}

func TestPaintTabInactiveHasNoPillFill(t *testing.T) {
	tb := testTabBar()
	thm := testTheme()
	thm.Palette.TabBarBG = color.NRGBA{R: 0x11, G: 0x11, B: 0x11, A: 0xff}
	thm.Palette.InactiveTabBG = color.NRGBA{R: 0xaa, G: 0xbb, B: 0xcc, A: 0xff}
	thm.Glass.FillAlpha = 1
	thm.Glass.RimAlpha = 0
	tab := newTestTab(t, 1, t.TempDir())

	buf := newBuf(200, 40)
	bar := toRGBA(thm.Palette.TabBarBG)
	raster.FillRect(buf, 200, 0, 0, 200, 40, bar)
	tb.paintTab(buf, 200, 40, thm, tab, 0, 100, 32, 3, false, false, false, false, false)

	pill := toRGBA(thm.Palette.InactiveTabBG)
	for y := 4; y < 28; y++ {
		for x := 10; x < 90; x++ {
			if pixelAt(buf, 200, x, y) == pill {
				t.Fatalf("inactive tab painted pill fill at (%d,%d); want plain title only", x, y)
			}
		}
	}
}

func TestPaintTabSeparatorsBetweenPlainTabs(t *testing.T) {
	tb := testTabBar()
	thm := testTheme()
	tabs := []session.Tab{
		newTestTab(t, 0, t.TempDir()),
		newTestTab(t, 1, t.TempDir()),
		newTestTab(t, 2, t.TempDir()),
	}
	buf := newBuf(400, 40)
	raster.FillRect(buf, 400, 0, 0, 400, 40, toRGBA(thm.Palette.TabBarBG))
	// Active = last tab → sticks between 0|1 only.
	tb.Layout(buf, 400, 40, 32, thm, tabs, tabs[2].ID, "", chrome.DragVisual{HoverIdx: -1}, false, true)

	// Expect a non-bar pixel somewhere near a slot boundary in the middle band.
	found := false
	bar := toRGBA(thm.Palette.TabBarBG)
	for y := 10; y < 22 && !found; y++ {
		for x := 1; x < 250; x++ {
			got := pixelAt(buf, 400, x, y)
			if got != bar && got.A != 0 {
				// Separator is a faint blend — not equal to bar.
				found = true
				break
			}
		}
	...
	if !found {
		t.Fatal("expected a separator stick between plain inactive tabs")
	}
}
...
func TestPaintTabUsesExplicitActiveBackground(t *testing.T) {
	tb := testTabBar()
	thm := testTheme()
	thm.Palette.ActiveTabBG = color.NRGBA{R: 0xaa, G: 0xbb, B: 0xcc, A: 0xff}
	thm.Palette.TabBarBG = color.NRGBA{R: 0x11, G: 0x11, B: 0x11, A: 0xff}
	thm.Glass.FillAlpha = 1 // opaque — assert exact ActiveTabBG pixels
	thm.Glass.RimAlpha = 0  // skip rim so interior scan isn't distracted
	tab := newTestTab(t, 1, t.TempDir())

	buf := newBuf(200, 40)
	// Fill bar first so we can distinguish pill pixels.
	raster.FillRect(buf, 200, 0, 0, 200, 40, toRGBA(thm.Palette.TabBarBG))
	tb.paintTab(buf, 200, 40, thm, tab, 0, 100, 32, 3, true, false, false, false, false)

	want := toRGBA(thm.Palette.ActiveTabBG)
	found := false
...
func TestLayoutPaintsTabsAndPlusButton(t *testing.T) {
	...
	tb.Layout(buf, 400, 40, 32, testTheme(), tabs, 1, "plugin status", drag, false, true)
...
func TestLayoutWithActiveDrag(t *testing.T) {
	...
	tb.Layout(buf, 400, 40, 32, testTheme(), tabs, 1, "", drag, true, true)
}

func TestCommandIndicatorColorNoneByDefault(t *testing.T) {
	tab := newTestTab(t, 1, t.TempDir())

	if _, ok := commandIndicatorColor(tab.Session, testTheme()); ok {
...
func TestCommandIndicatorColorWhileRunning(t *testing.T) {
	...
	got, ok := commandIndicatorColor(tab.Session, thm)
	if !ok {
		t.Fatal("a running command should show an indicator")
	}
	if want := toRGBA(thm.UI.CommandRunning); got != want {
		t.Fatalf("running indicator color = %+v, want %+v", got, want)
	}
}
...
func TestCommandIndicatorColorAfterSuccessAndFailure(t *testing.T) {
	cases := []struct {
		name    string
		doneSeq string
		want    func(theme.Theme) color.NRGBA
	}{
		{"success", emu.OSC133CommandDone, func(thm theme.Theme) color.NRGBA { return thm.UI.CommandSuccess }},
		{"failure", emu.OSC133CommandDone1, func(thm theme.Theme) color.NRGBA { return thm.UI.CommandFailed }},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if want := toRGBA(c.want(thm)); got != want {
				t.Fatalf("%s indicator color = %+v, want %+v", c.name, got, want)
			}
		})
	}
}
