...
// OSC52Handler lets the host application intercept OSC 52 clipboard
// sequences (CSI ]52;Pc;Pd ST/BEL) instead of the payload being decoded
// and silently discarded, which is upstream cy/pkg/emu's stock behavior
// (see handleOSC52 in parse.go — its own "TODO(cfoust): publish event"
// comment already anticipated exactly this). This interface and
// WithOSC52Handler are a geckty addition, not part of upstream.
type OSC52Handler interface {
	// OSC52Write is called when the child process asks to set clipboard
	// pc (only "c", the system clipboard, is recognized — "p"/primary
	// and "s"/select selections are not) to data.
	OSC52Write(pc string, data []byte)
	// OSC52Read is called when the child queries clipboard pc. ok=false
	// means "nothing to report", and no response is sent at all — this
	// matches how real terminals behave when clipboard access is
	// unavailable or denied, rather than sending an empty response.
	OSC52Read(pc string) (data []byte, ok bool)
}

// WithOSC52Handler installs h to receive OSC 52 clipboard write/query
// requests. Without it, OSC 52 sequences are parsed (so they don't
// corrupt grid state) but otherwise ignored — see OSC52Handler.
func WithOSC52Handler(h OSC52Handler) TerminalOption {
	return func(info *TerminalInfo) {
		info.osc52 = h
	}
}
