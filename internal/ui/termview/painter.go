...
type Painter struct {
	...
	fallbackAtlas *raster.GlyphAtlas
}

// SetSymbolFallback installs a face used for glyphs missing from the
// primary mono family and clears the fallback glyph atlas cache.
func (p *Painter) SetSymbolFallback(face font.Face) {
	p.fallbackFace = face
	p.fallbackAtlas = nil
}
...
func (p *Painter) ensureAtlas() {
	...
	for i, f := range faces {
		...
		if !p.atlases[i].Valid(f, p.Ascent) {
			if i == 0 {
				// Regular: slight stem fatten so default text matches the
				// "boldish" look of Menlo/SF Mono Medium in other apps.
				p.atlases[i] = raster.NewEmboldenedGlyphAtlas(f, p.Ascent)
			} else {
				p.atlases[i] = raster.NewGlyphAtlas(f, p.Ascent)
			}
		}
	}
	if p.fallbackFace != nil && !p.fallbackAtlas.Valid(p.fallbackFace, p.Ascent) {
		p.fallbackAtlas = raster.NewGlyphAtlas(p.fallbackFace, p.Ascent)
	}
}
...
func (p *Painter) faceAndAtlas(bold, italic bool) (font.Face, *raster.GlyphAtlas) {
...
func (p *Painter) glyphEntryFor(bold, italic bool, r rune) (raster.GlyphEntry, bool) {
...
func (p *Painter) Paint(buf []byte, frameW, frameH, originX, originY int, term *vt.Terminal, scrollOffset int, sel Selection, placements []Placement, blinkOn bool, dirtyRows vt.DirtyRows) bool {
	...
	if dirtyRows == nil {
		raster.FillRect(buf, frameW, originX, originY, originX+gridW, originY+gridH, bg)
		...
		for row := range dirtyRows {
			...
			if sel.Active {
				...
				if ok {
					...
					if highlight.A == 0 {
						highlight = theme.DefaultSelection
					}
...
func (p *Painter) paintRow(buf []byte, frameW, frameH int, line emu.Line, cols, originX, y int, sel Selection, viewRow int) {
	...
	for col := 0; col < cols; {
		...
		if bgColor != ToRGBA(p.Palette.Background) {
			raster.FillRect(buf, frameW, x0, y, x1, y1, bgColor)
		}
		...
		if st.underline {
			...
			linkUL := ToRGBA(p.URLUnderline)
			if p.URLUnderline.A == 0 {
		...
		if st.strikethrough {
			sy := y + p.CellHeight/2
			raster.FillRect(buf, frameW, x0, sy, x1, sy+1, fgColor)
		}
...
func paintUnderline(buf []byte, frameW, x0, x1, y1 int, mode emu.UnderlineMode, fg color.RGBA) {
	...
	case emu.UnderlineDouble:
		raster.FillRect(buf, frameW, x0, uy, x1, uy+thickness, fg)
		raster.FillRect(buf, frameW, x0, uy-2, x1, uy-2+thickness, fg)
	case emu.UnderlineDotted:
		for x := x0; x < x1; x += 2 {
			...
			raster.FillRect(buf, frameW, x, uy, xe, uy+thickness, fg)
		}
	...
	case emu.UnderlineCurly:
		...
		for x := x0; x < x1; x++ {
			...
			y := base + dy
			raster.FillRect(buf, frameW, x, y, x+1, y+thickness, fg)
		}
	default:
		raster.FillRect(buf, frameW, x0, uy, x1, uy+thickness, fg)
	}
...
func (p *Painter) paintCursor(buf []byte, frameW int, term *vt.Terminal, originX, originY int) {
	...
	case emu.CursorStyleUnderline, emu.CursorStyleBlinkUnderline:
		const thickness = 2
		raster.FillRect(buf, frameW, x0, y0+p.CellHeight-thickness, x0+p.CellWidth, y0+p.CellHeight, fg)
	case emu.CursorStyleBar, emu.CursorStyleBlinkBar:
		const thickness = 2
		raster.FillRect(buf, frameW, x0, y0, x0+thickness, y0+p.CellHeight, fg)
	default:
		raster.FillRect(buf, frameW, x0, y0, x0+p.CellWidth, y0+p.CellHeight, fg)
	}
}
...
func (p *Painter) paintPlacements(buf []byte, frameW, frameH int, placements []Placement, top, rows, originX, originY, gridW int) {
	for _, pl := range placements {
		...
		raster.BlitImageScaled(buf, frameW, frameH, pl.Image, x0, y0, w, h)
	}
}
...
// ToRGBA converts an NRGBA color to RGBA for buffer fills.
func ToRGBA(c color.NRGBA) color.RGBA {
	return color.RGBA(c)
}

// WithAlpha returns c with alpha replaced by a.
