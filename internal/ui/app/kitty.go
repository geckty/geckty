...
// built once from keymap.go's input.SingleCharBindings (its inverse).
var keyToChar = func() map[gpucontext.Key]string {
	m := make(map[gpucontext.Key]string, len(input.SingleCharBindings))
	for s, k := range input.SingleCharBindings {
		m[k] = s
	}
	return m
}()
