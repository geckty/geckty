...
func (s *uiState) paintSearchOverlay(fw, fh, padPx, _ int) {
	...
	bg := toRGBA(s.thm.Palette.InactiveTabBG)
	raster.FillRect(s.frame, fw, 0, y0, fw, fh, bg)
	...
	fg := toRGBA(s.thm.Palette.Foreground)
	if s.tabBar != nil {
	...
	hi := toRGBA(s.thm.UI.SearchMatch)
	raster.BlendRect(s.frame, fw, x0, yHit, x1, yHit+s.cellH, hi)
}
