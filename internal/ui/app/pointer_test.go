...
func TestCellFromPositionSubtractsOrigin(t *testing.T) {
	// col 2 needs x = originX + 2*cellWidth; row 1 needs
	// y = originY + 1*cellHeight. originY includes chrome+pad.
	col, row := cellFromPosition(3+20, 45+20, 10, 20, 3, 45, 80, 24)
	if col != 2 || row != 1 {
...
func TestCellFromPositionZeroColsRowsSkipsUpperClamp(t *testing.T) {
	...
	col, row := cellFromPosition(1000, 1000, 10, 20, 40, 5, 0, 0)
	if col < 0 || row < 0 {
