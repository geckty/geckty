...
func TestStrokeRoundRectPaintsRingNotInterior(t *testing.T) {
	buf := newBuf(20, 20)
	StrokeRoundRect(buf, 20, 2, 2, 18, 18, 4, color.RGBA{R: 0xff, G: 0xff, B: 0xff, A: 0xff})
	if got := pixelAt(buf, 20, 10, 10); got.R != 0 {
		t.Fatalf("interior (10,10) = %v, want untouched (stroke only)", got)
	}
	// Centered SDF stroke paints on the boundary (x≈2).
	if got := pixelAt(buf, 20, 2, 10); got.R == 0 {
		t.Fatalf("edge (2,10) = %v, want rim paint", got)
	}
}
