...
func TestFillRectFillsCorrectRegion(t *testing.T) {
	...
	raster.FillRect(buf, 4, 1, 1, 3, 3, red)
...
func TestFillRectClampsToBufferBounds(t *testing.T) {
	...
	raster.FillRect(buf, 2, -5, -5, 100, 100, color.RGBA{G: 0xff, A: 0xff})
	for y := 0; y < 2; y++ {
		for x := 0; x < 2; x++ {
...
func TestBlitGlyphClippedRespectsClipBounds(t *testing.T) {
	...
	raster.BlitGlyphClipped(buf, 6, 3, dr, mask, image.Point{}, fg, 2, 0, 4, 3)
...
func TestBlitGlyphClippedSkipsFullyTransparentPixels(t *testing.T) {
	...
	raster.BlitGlyphClipped(buf, 2, 1, dr, mask, image.Point{}, fg, 0, 0, 2, 1)
